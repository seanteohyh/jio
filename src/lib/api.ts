...
export function errorResponse(error: unknown): NextResponse {
  ...
  if (/only the host|can add places|can remove|not allowed|cannot/i.test(message)) {
    return forbidden(message);
  }
  if (/not found|not valid/i.test(message)) return notFound(message);
  if (/already|invalid|required|must be/i.test(message)) {
