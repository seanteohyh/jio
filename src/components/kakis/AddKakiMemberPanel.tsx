...
export default function AddKakiMemberPanel({
  ...
  const { data } = useSWR<{ users: TeamUser[] }>("/api/users", fetcher);

  const results = useMemo(() => {
    const needle = query.trim().toLowerCase();
    return (data?.users ?? [])
      .filter((u) => !existingMemberIds.includes(u.user_id))
      .filter(
        (u) => !needle || u.display_name.toLowerCase().includes(needle)
      );
  }, [data, existingMemberIds, query]);
        ...
        <div className="max-h-48 space-y-1 overflow-y-auto">
          {results.length === 0 && (
            <p className="text-stone text-xs">
              Nobody matching &ldquo;{query}&rdquo;.
            </p>
          )}
