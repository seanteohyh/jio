...
export interface LunchEvent {
  ...
  /** Set by `editEventWinner` ("Where did you actually go?") — the group's
   *  own ballots never decided this result, a host corrected it after the
   *  fact. `null` on reopen (a fresh close cycle). The chit and Final count
   *  both swap the winner's points for "Overruled" once this is set,
   *  rather than attributing a vote count to a result nobody voted for. */
  winner_corrected_at?: string | null;
  kaki_id?: string | null;
