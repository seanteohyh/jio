...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (config.isDemo) {
      ...
      // would be rejected by the places INSERT policy (created_by = auth.uid()).
      // This is the one and only caller allowed to bypass RLS.
    ...
    // The keep-alive already succeeded, so report a partial success rather
    // than a 500 that would show up as a failed cron run.
    return json(
      {
        fetched: 0,
        new: 0,
