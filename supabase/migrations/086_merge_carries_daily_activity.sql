-- 086_merge_carries_daily_activity.sql
--
-- Root cause of "Preview says nothing to move, Confirm merge does nothing,
-- the duplicate never disappears": migration 076 added `app_daily_visits`
-- and `action_events`, both `user_id uuid not null references auth.users(id)`
-- with no `on delete cascade`/`set null` — and `merge_user_accounts` (last
-- redefined in 044, before 076 existed) was never taught about them. Every
-- signed-in page view writes an `app_daily_visits` row, so any real account
-- being merged almost certainly has one. `merge_user_accounts` itself never
-- touches these tables, so it reports success — but the later
-- `admin.auth.admin.deleteUser(mergeUserId)` call then hits a plain
-- Postgres foreign-key violation trying to delete the now-orphaned
-- `auth.users` row, which is exactly the failure the merge UI was
-- surfacing (previously as an uninformative literal "{}").
--
-- `create or replace` on the same signature, full body carried forward
-- from 044 plus the two new tables — same "carry forward, add the new
-- block" shape as 043/044.
...
begin
  ...
  -- app_daily_visits (076) — PK (user_id, visit_date), and its `user_id`
  -- has a hard FK to auth.users with no cascade/set-null. Every signed-in
  -- page view writes a row here, so leaving this untouched is what was
  -- actually blocking the old account's retirement (a plain Postgres
  -- foreign-key violation on the later `auth.users` delete, reported to
  -- the admin as an opaque failure). Where both accounts visited on the
  -- same day, merge the counts instead of just dropping one side's history.
  ...
  -- action_events (076) — own `id` PK, no collision possible. Same FK gap
  -- as app_daily_visits above.
  update action_events set user_id = p_keep_user_id
    where user_id = p_merge_user_id;

  -- admin_engagement_weights.updated_by (064) — singleton row, same FK
  -- gap; only ever set to whichever admin last changed the weights, but
  -- cheap to keep consistent rather than leave a dangling reference.
