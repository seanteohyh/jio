...
3. **SQL Editor** — run every file in `supabase/migrations/` in numeric order,
   001 through 066. They are idempotent, so re-running is harmless.
...
```bash
npm test          # 583 tests across 50 files
npm run typecheck
npm run lint
```
