...
```bash
npm test          # 886 tests across 71 files
npm run typecheck
npm run lint
```
...
| `expenseTracking.test.ts` | The personal expense ledger: logs an entry defaulting `logged_at` to today; lists only the calling user's entries for a given month, newest first; paginates 20/page with no overlap between pages; amends an entry in place (confirmed: edit, not delete-only); refuses editing or deleting someone else's entry; an entry survives its source visit being deleted, just unlinked (confirmed: `on delete set null`, not cascade); `getExpenseMonthSummary` totals by category, computes a delta against the previous month (`null` with nothing to compare), and writes a plain-language comparison note against the account's own budget preference; `centsToBudgetTier`'s reverse-mapping of the budget-tier dollar bands |
