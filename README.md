...
```bash
npm test          # 841 tests across 69 files
npm run typecheck
npm run lint
```
...
| `recurringSeries.test.ts` | Recurring-series date math (lookahead window, no double-generation), fixed vs. voted occurrences, fresh kaki-membership expansion per occurrence, each generated occurrence inheriting the series' own vote-deadline offset, and a deadline change propagating onto an already-generated open occurrence even after someone's voted (unlike place/mode/invitees) |
