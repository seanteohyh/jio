...
3. **SQL Editor** — run every file in `supabase/migrations/` in numeric order,
   001 through 067. They are idempotent, so re-running is harmless.
...
it's sensitive like `recovery_token` — `first_decided_celebration_shown_at`
(migration 067) is an ordinary one-shot flag anyone could see — but because
041's column restriction applies to any column, granted or not; skip the
grant and it's silently unreadable through `getProfile()`'s explicit column
list rather than just omitted.
...
```bash
npm test          # 594 tests across 51 files
npm run typecheck
npm run lint
```
...
| `publicEventPreview.test.ts` | The public event-invite preview: safe subset for a real event, `goingCount` only counts confirmed "yes" RSVPs (not maybe/no), never exposes votes/tally/invitees/RSVPs, redacted the same regardless of a Jio's own `hide_votes` setting, `null` for an unknown token |
...
| `firstDecidedCelebration.test.ts` | The one-time "first decided Jio" celebration's qualifying condition — closed + any RSVP answer + a cast vote, never once already shown, unaffected by which RSVP answer it was — `markFirstDecidedCelebrationShown`'s idempotent stamp (no-op on a second call or an unknown user), and an end-to-end check wired through a real `closeEvent` call, including that a non-voter's RSVP alone doesn't qualify |
