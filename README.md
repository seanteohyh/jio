...
| | |
|---|---|
| **Jios** | Create a lunch outing, everyone ranks the options, a Borda count decides. RSVP, live vote updates, a roulette wheel for when the group genuinely cannot choose, and a month calendar with Hosting / Invited / Past filters for browsing, plus a Calendar / List segmented toggle labeled by which view is active rather than which one tapping it switches to. RSVPing itself — not just seeing who answered what — stays available after the Jio closes too: "Who's coming" breaks the list down into Going / Maybe / Can't make it for everyone to see, not just the host, and the "I'm in" / "Maybe" / "Can't" buttons stay live regardless of open or closed, since a host inviting someone new post-decision (below) would otherwise have no way for that person to actually answer. A host can also add or remove invitees at any time, before or after, from an "Invited" section on the Jio itself (host-only; a linked Kaki's members aren't individually listed there — they come and go with the group; a plain-language note there says "Only the host can add people to this Jio," UX review log #19, since that default was never communicated anywhere in the app). A decided Jio whose date is still ahead reads "Going to X," not "Went to X" — closing the vote early doesn't make the lunch itself any less upcoming, and both the Jios list's own "Up next" and Home's "Upcoming" (above) key off the same real-date logic rather than treating `closed` as synonymous with "already happened." Removing someone also drops their own RSVP, ballot and any Flexi date-availability on that Jio, so a stray response doesn't linger for someone no longer in it; whatever they added (an option, a candidate date) stays for everyone else. A Flexi Jio also takes a shared time when its candidate dates are set (noon by default) — the same time carries onto whichever date the host later confirms, built via an explicit `+08:00` offset rather than left as a bare "YYYY-MM-DD," which always parses as UTC midnight (8am once shown in Singapore time). Anyone can edit a place, any signed-in user may edit any place's details, and the host can cancel a Jio outright — it stays visible, marked cancelled, rather than vanishing. Once a Jio is decided, its result — place, and a top-3 vote breakdown as a compact bar chart (the winner's row in ember with its point count folded into the bar itself, the rest in one shared muted tone, capped at 3 rows and omitted entirely if nobody voted at all) — can be copied or shared as a PNG, generated client-side, for pasting straight into a chat instead of a link. Sharing carries a plain "Jio confirmed!" caption alongside the image, deliberately *not* the winner's Google Maps link — a shared link in the caption text reliably makes at least one common share target (WhatsApp) drop the attached image entirely and show only a link preview instead, worse than the image alone. The Maps link is offered as its own "Copy Maps link" action next to Share/Download instead, so it can still go out as its own message rather than silently costing the image. A place name is a link wherever it's shown while voting or once decided — Your ranking, Standing, and the Decided banner itself — with a small map-pin icon next to it opening Google Maps in a new tab, and a second icon alongside it (Instagram, Facebook, or a plain link glyph, sniffed from the URL's domain) when that place has a socials link set (CHANGES_20260821b.md §1); a free-text option with no real place behind it stays plain text, nothing to link to. The host can also correct either side of a Jio after the fact: its date/time, at any point short of cancelled (even after it's closed — using this on a still-polling Flexi Jio finalizes the date the same way confirming a candidate does, just not limited to the pre-listed candidates), and once closed, which place it actually ended up at ("Where did you actually go?"), accepting any real place rather than only one that was ever a voting option. The winner correction only touches the Jio's own record — it doesn't retroactively log a visit or move anyone's personal metrics. The Jio's own name is a third correction in "Edit this Jio," ("Change name") — purely cosmetic, so unlike the date/time and winner corrections it stays available at any time at all, including once cancelled, with no state-machine gate of its own. A host can also undo a close outright — "Reopen for voting" — while the Jio's scheduled time is still ahead of it; existing ballots aren't touched (a vote already persists until its owner recasts it, same as any other voting), only the recorded winner and close time are cleared, since closing again recomputes both. Unlike the winner correction, this is a structural state change back to `open`, so it goes through its own gated database function rather than a plain write, the same reasoning behind the dedicated function `cancelled` already gets. Reopen works the same afterward regardless of how the Jio closed — by the host's own hand or the vote-deadline sweep below. A Jio used to close itself (CHANGES_20260821_combined.md Part 2), no host action required, the moment every participant had RSVP'd Yes or Can't (a lingering Maybe, same as silence, kept it open) and everyone who said Yes had cast a ballot. A confirmed real bug in that full-consensus path — "Jio keeps closing whilst new members trying to add their votes in" — was more fundamental than a fixable race: a Jio anyone can still join, via its own share link or (kaki-linked) just by being a member, can never have a genuinely final population, so "everyone's answered" is a moving target. Someone opens the link, joins, votes, and if everyone *already* joined had answered, the Jio closed right then, before whoever was about to open that same link a moment later ever got the chance. It doesn't auto-close any more. The same condition (every participant answered, every Yes backed by a fresh vote) is still computed, live, on every page load — `EventDetail.readyToClose` — but only ever surfaced as a prompt right under the share link on the host's own page ("Everyone's answered — ready to close?", with its own "Close with the vote" button), not acted on by itself; a new joiner un-readies it the instant they show up, same as it would've blocked the old auto-close from ever having fired in the first place. The host also gets pushed the same prompt (reusing the existing "someone voted" push and its throttle) the moment an RSVP or a vote makes this newly true. A host can also set a vote deadline at creation ("Voting closes," a duration picker — 30 min up to 24h before the Jio's own start time, defaulting to 3h, or "No deadline") that force-closes voting once it passes, regardless of who's still pending — genuinely automatic, and safe in a way full-consensus never was, since a deadline is a fixed point in time, not "whoever's joined so far." The deadline is editable any time the Jio's still open ("Edit this Jio"), shown as a live countdown on the Jio page itself and in the signed-out `/e/[token]` preview, and mentioned in the invite push. A Flexi Jio takes the same setting at creation but only resolves an actual deadline once its date is confirmed; a recurring series carries its own configured offset onto every occurrence it generates, propagated onto an already-generated still-open one the same way `time_of_day` already is (unlike place/mode/invitees, moving a deadline doesn't invalidate anyone's existing answer). Enforcement is a new cron sweep (`/api/cron/vote-deadline`, same external-scheduler requirement as the "starting soon" reminder — see Free-tier realities) that also sends a one-time "voting closes soon" push to whoever's still pending, an hour out by default. As soon as the date is fixed — for a Flexi Jio, once its poll resolves; otherwise from the moment it's created — "Add to calendar" unlocks: a one-tap Google Calendar link and a `.ics` download, deliberately available before the Jio closes or a place wins (so nobody has to wait on the group just to block their own calendar), showing "Place TBD" until a place is decided. Both are a one-time snapshot at a fixed 1-hour duration rather than a live sync — if the Jio's time or place changes afterward, an already-added calendar entry doesn't follow. It's unavailable only once a Jio is cancelled. A host can also start a Jio with votes hidden: nobody, host included, sees the running standing until it closes — only the ballot count shows. Whether votes are hidden can also be toggled after the fact, either direction, in "Edit this Jio" while it's still open — a recurring Jio's occurrences have no creation-time form of their own to set `hide_votes` on up front, so this "Hide votes" / "Reveal votes now" toggle is the only way a host ever hides one of those, and hiding even once some votes are already visible is fine, since anyone can still revote. A plain host-gated column write same as the winner correction rather than its own `SECURITY DEFINER` function, unlike `cancelled`/reopen. Each option in the Standing and Your ranking lists shows a compact cuisine descriptor beneath its name, so a placeless or unfamiliar option still says something about what it is before anyone clicks in. Following a shared event invite link (`/e/[token]`) while signed out now shows a narrow preview first (CHANGES_20260821_combined2.md §3A) — host name, a rough going-count, and the place options as plain chips, no votes/invitee identities/RSVP names, same "unguessable token, SECURITY DEFINER resolver" shape as `/p/[id]`/`/l/[token]` — with "Sign in to join" as the one way through; a signed-in visitor still gets bounced straight to the real event page as before. An account's first-ever "Start a Jio" tap (CHANGES_20260821_combined2.md §3C) also pre-checks a co-attendee list — the host plus everyone invited to the most recent Jio this account joined as a guest — in "Who is coming," rather than starting from the usual empty picker; each name is still a removable chip, and the search field is still right there to add anyone else. This is gated on having never hosted before, not on being new — the very next hosting attempt, and every one after it, gets today's ordinary empty/ranked picker. Any Jio an account RSVP'd (any answer) and voted on, once decided *and* still upcoming, gets a one-time celebration above the everyday "Decided" card on that Jio's own page (originally first-Jio-only — CHANGES_20260821_combined2.md §3D — generalised to every qualifying Jio by UX review log #25): three pebbles converge from scattered positions into the real JioMark's exact path geometry against a coffee-toned backdrop, its centre dot recoloured a lighter warm tan (a local override of JioMark's own `--color-espresso` variable, scoped to just this one usage — true espresso-on-espresso would be invisible against the backdrop) rather than a separately-sized stand-in disc, which only ever produced either a hidden or a ringed circle once the real mark settled on top of it, with a plain-text `aria-live` announcement riding the same trigger. A full-screen splash, not an inline page card — a real report that the card version could land scrolled out of view (and its own entry animation missed with it) below wherever the page happened to be scrolled after the vote or RSVP that triggered it — dismissed by its own explicit "Continue" button back to the Jio underneath, never a timer or a backdrop tap. Not gated on catching the live open-to-closed transition, since most Jios close while nobody's watching (the vote-deadline sweep, or the host closing it) — it fires on whichever qualifying decided Jio's page this account next happens to load, stamped immediately (one row per (user, event), not an account-wide flag) so it can never show twice, and a Jio whose lunch has already passed by the time it's seen goes straight to the resting record instead. The shareable result card is a hawker-order-chit-styled ticket, not a plain rectangle — perforated notches, a dashed divider, serif headline type and the app's own logo watermarked into the header, all drawn straight onto the canvas — with "Copy Jio link" as its own button alongside the existing Copy image/Download/Copy Maps link actions. The create form also takes an optional free-text notes field (parking, dress code, anything else worth knowing) — shown to every invitee on the Jio's own page and in the signed-out `/e/[token]` preview alike, set once at creation with no edit path. Any place option can also carry its own short note ("opens at 12:30pm instead") — distinct from the Jio-wide notes above, since this is context about one specific place, not the outing overall. Only whoever added that option may set or clear it (deliberately narrower than removing an option, which the host can also do regardless of who added it — a note is someone claiming to know something specific, not a moderation action), edited inline right where an added-by-you place already shows in "Add a place," and surfaced everywhere that place appears while voting: Standing, Your ranking, and the add-a-place list itself. Also settable at creation, not just after — the create form's place picker grows the same per-place note input beneath whatever's currently selected (fixed-date Jios only; a Flexi Jio has no place options until its date resolves), reusing `setOptionNote` server-side right after `createEvent` rather than threading a notes map through that already-long parameter list. Bug report item 4 — a new place added to an open Jio now marks every already-cast "yes" ballot stale for `readyToClose` purposes: `options_changed_at` bumps whenever a place is added, and a ballot cast before that point no longer counts as "voted" until its owner recasts it (even the same ranking), with a plain banner on the Jio page — "A new place was added — recast your vote so it counts" — telling them why. This only ever gates the `readyToClose` prompt; the vote-deadline sweep is deliberately unaffected and still closes with whatever ballots exist once its own deadline passes, staleness or not, so a bounded close is never blocked by someone who never comes back to revote. The winner correction's place search only ever matched real places (`winner_place_id` has to resolve to one — Maps links, budget info, everywhere else that joins on it), so typing the exact name of a free-text ("not here? add it anyway") vote option that actually won found nothing and silently did nothing, with no indication why. The search now recognizes when the query matches one of this Jio's own free-text options and says so directly, linking straight to registering it as a real place (`/places/new?...&fromEvent=...&draftPlaceId=...`, the same "attach on save" flow a free-text option's own persistent link already uses) — search again afterward and it's a real candidate. A query matching neither a real place nor a free-text option gets a plain "add a new place" fallback instead of silence either way. Correcting the winner also stamps `winner_corrected_at` (cleared on reopen, same as `winner_place_id`/`closed_at`) — the group's own ballots never decided this result, so both the Final count list's winner row and the share chit's summary line swap the points clause for "Overruled" once it's set, rather than attributing a vote count (possibly zero, possibly for a place that was never even an option) to a result nobody actually voted for. |
...
| **Kakis** | Lunch groups with a shareable invite link and shared stats — group favourites, who eats out most, who is most adventurous, with the same dashed-border empty state as everywhere else once nobody's logged a visit yet. Any current member can also add someone directly by searching their name, without a link changing hands first — same trust level as the invite link itself, just faster for someone already sitting across the table. Leaving a group asks for confirmation first, same as cancelling a Jio — there's no path back short of a new invite. "Who eats out most" and "who is most adventurous" now live on a dedicated group-vibe card (CHANGES_20260821_combined2.md Item 1) rather than as plain stat tiles — a locked monthly snapshot, same cadence and share button as the personal food identity card, alongside a third "Trailblazer" award for whoever's own visit history includes the most places no other member has ever been to (distinct from raw visit count or raw distinct-place count, either of which tends to just reward whoever eats out the most). The card's own cuisine/spend "vibe" numbers are weighed equally across members rather than pooled by raw visit count, so whoever logs the most visits doesn't silently become "the group's taste" — a member with 3 visits counts exactly as much as one with 50; the literal totals (total visits, distinct places, favourite places) stay pooled, since those are genuinely about collective volume, not opinion. Tapping any award row expands it into that award's full per-member ranking, not just the single winner — computed live off the group's current visits, independent of whichever month's headline/description is locked in. Who's actually shown as crowned in each slot also runs through a "home kaki" rule: someone who's simply the most active/adventurous/novel person across several of their groups is crowned only in their single best one (with a small "🔥 N-kaki streak" badge once that's 2+), while every other group where they'd also be #1 quietly promotes its own runner-up instead — shown with a small italic note ("[Leader]'s leading here too … — crowned in [group]") rather than silently. This is resolved fresh on every page load (nothing about it is persisted), so a changing streak takes effect immediately with no cooldown. A "Fresh reviews" card sits at the bottom of the page, right after group favourites — the last 3 public reviews from any member, anywhere, not limited to somewhere the group has actually been together (a broader "what are my Kaki-mates eating lately" than the group-favourites stats already give). Each one links through to its place and carries the same like button reviews get everywhere else. Private visits never surface here, same privacy floor as the rest of this page. Creating a new group can now invite others straight away — the same name-and-invite picker a Jio uses, minus the option to pick a whole other group (adding a group to a group makes no sense). A decided Jio also grows a "Turn this into a Kaki?" card once its exact group of participants doesn't already share one — host-only, and gone for good the moment either the host dismisses it or actually creates the group (accepting doesn't write anything special; the suggestion just stops finding a gap to fill). Its own "Turn into a Kaki" button lands on /kakis with that exact group pre-checked in the picker, so the host can still remove someone before confirming. Any current member can rename the group, not just whoever created it — same trust level as adding someone directly. A **Wishlist card** sits right after the group-vibe card, one step below it: a shared, group-owned counterpart to the personal wishlist (`kaki_wishlist_entries`, migration 093) — "we should try that sometime" said once in the group chat now actually survives, since anyone in the group can add a place (search-to-add, same place search the recurring-Jio and rating forms already use) or clear one, not just whoever added it — same "any member is trusted with the group's shared state" reasoning renaming the group itself already runs on. Each row's "Jio this" jumps straight into the same inline `JioForm` `HomeHero` already uses, with that place and this Kaki pre-selected — wiring onto existing form props (`initialPlaceIds`/`initialInvite`), not new form logic. Filtered to the place still being `active` wherever it's listed (confirmed: filtered from view, not pruned from the table, if a listed place later gets blocked — same convention the Jio "Suggest 3" ranking already applies) — recoverable if the place is ever unblocked. That same ranking (`pickCommitteeSuggestions`) also gives a kaki-wishlisted place the same bump toward "personalized" an individual's own wishlist entry already gets, whenever the Jio being suggested for actually belongs to a Kaki — additive, decided locally rather than threaded through the deeper per-member scoring `groupRecommend` itself runs, since a group's shared wishlist is a different signal from any one member's personal one. |

| **Spending** | A private, per-user spending ledger (`expense_entries`, migration 094) — deliberately not the same shape as a `Visit`: most entries have no formal `Place` at all (a coffee run, a snack from an unlisted stall), so `place_id` is nullable and what was bought is a free-text label — but never *only* a lookup-free label either, since it doubles as an opportunistic place search (migration 095 also widened `category` from lunch/coffee/snack/other to add **breakfast** and **dinner**, six in total). "What was it?" runs the exact same `/api/places?q=` search every other search box in the app already uses, live under the field; picking a suggestion fills the label with that place's own name and links `place_id`, exactly the way `Visit`-created entries already do — but nothing forces a match. Typing straight past the dropdown, or editing the text again after a pick (which clears the just-made link, so a stale match never silently rides along with newly typed text), saves whatever's there as a plain label with no place attached, same as before this existed. The same search-then-fallback now also runs on an edit, not just a new entry, so a match missed at capture time (or one that needs correcting) isn't stuck that way forever. Two ways in: an optional amount right on the existing "rate this place" form (`/places/[id]`, shown only when logging a brand-new visit, never an edit) — logs a `lunch`-category entry tied back to that visit via `source_visit_id`; and a standing quick-add on the `/profile/spending` page itself, reachable in two taps from "You" → the "Usual spend" tile. That form's own field reads "How much did you personally spend?", not the vaguer "How much did it cost?" it originally shipped with — real usage found reviewers reading "it cost" as the whole table's bill (this being a review form, not obviously a personal-tracker one), and logging a group total instead of their own share; the hint spells it out further ("just for your own private spending tracker — not the group's bill, and never shown on the review"). `/profile/spending` itself opens on an **"Insights"** tab by default, not the personal ledger — a budget-tier breakdown (`BudgetTierInsights`, shared with `/profile/visits?view=spend`) computed entirely from places already visited, needing not one logged amount — since most people will never manually track a dollar figure, and landing them on an empty ledger first (Jio's "warm," not-another-chore philosophy) undersold what the app already knows about their spending without asking for anything. A second **"My tracker"** tab holds everything below — the month nav, quick-add and entry list — one tap away, not removed, just no longer the first thing shown; the "Usual spend" tile itself links straight into "My tracker" (`?tab=tracker`) once it already knows real entries exist, so someone already using it isn't demoted behind an extra tap every time. The page also carries a "← Back to You" link at the top, matching every other `/profile/*` subpage. That page is a real route, not another profile card: a month filter (« August 2026 », next disabled once back at the current month — nothing logged before someone starts using this, so no lower bound needed beyond that), a summary card (month total, a by-kind bar breakdown, and a plain-language sentence comparing the month's average entry against the account's own budget preference, reusing `budgetLabel()`'s `$`-tier formatting rather than a bare dollar comparison), and the entry list itself — every entry that month, newest first, **numbered pages** (20 per page, confirmed over infinite scroll — pairs with the month filter's own bounded-unit paging, and the first user-facing pagination pattern in the app). Each row supports both **edit and delete** (confirmed — typo'd amounts and mis-tapped categories are common enough that delete-and-re-add alone wasn't worth shipping as final), with "· From a review" the only trace of an entry created via the rating-form path — the list otherwise treats every entry the same regardless of how it was logged. An entry whose source visit is later edited or deleted survives untouched, just unlinked (confirmed: `source_visit_id` is `on delete set null`, not cascade — a personal ledger entry never silently disappears because of something that happened to a review, though the two can drift apart, e.g. the visit's date moving without the expense's `logged_at` following it). Always strictly private — no policy on `expense_entries` ever grants read access to anyone but the row's own `user_id`, unlike the wishlist above or a `Visit`'s own `is_public` split; this is the one piece of financial-shaped data in the app that never becomes a shared or group-aggregate number the way `avgBudgetTier` already is. Deliberately not built yet: teaching `avgBudgetTier`/`groupAvgBudgetTier`/the `budget_hunter` Food Identity archetype to prefer a real logged average over a place's fixed price tier once enough amounts exist — safe to defer, since the ledger is useful entirely on its own first. |
...
| **Lobangs** | "Saw this online, thought of you" — send any registered place to specific teammates, a whole Kaki, or — one tap over — anyone at all, with an optional note and personalized "quick pick" suggestions for a teammate/Kaki send. "Share this lobang" is the one entry point on a place's own page (`/places/[id]`), same composer either way; a past Jio's "You → Past Jios" is the other, with the winning place pinned as a default. A teammate/Kaki send is tracked, shows up in the recipient's `/lobangs` feed, and push-notifies them — and once it's sent, an optional "Let them know" step offers the place's own public page (`/p/[id]`, no sign-in needed to view it) to share or copy, a manual nudge for anyone who won't notice the in-app inbox or a push right away; the "Anyone" path instead mints an attributed `/l/[token]` link — the sender's real name and note, resolved server-side so nobody can forge one — that anyone can open, signed in or not, and never appears in anyone's Lobangs feed or triggers a push. A dedicated `/lobangs` page (linked from "See all" on the profile inbox) shows every *targeted* send, received and sent, as one reverse-chronological, message-bubble feed — your own sends right-aligned, a group send showing the Kaki's name as the recipient. A received lobang takes a heart (toggle, per-recipient, same shape as a review like), "Start a Jio here" (a one-tap shortcut straight into `/events/new?placeId=` with that exact place already selected), and a comment thread — every lobang, however it was sent, has exactly one shared thread (`lobang_comments`, `096_lobang_comments.sql`) that the sender and every recipient of that send can all read and post into, back and forth, like a comments section under the lobang itself (a group send to a whole Kaki means one thread the sender and every member of that send share, not a private one per recipient). This replaced an earlier model — one freeform text reply per recipient, silently overwritten by a second reply rather than threaded, and postable only by the recipient, never the sender — after a report ("tried to reply a reply for a review, got a white screen") traced back to a genuine gap: no reply/comment feature existed anywhere in the app in the shape people expected, and the one "reply" that did exist could never hold more than one message or go both ways. Each lobang card shows a running "N comments" count that expands the thread in place; posting notifies (push, throttled the same way `038_vote_push_throttle.sql`'s vote pushes are) everyone else in that thread. Liking, Jio-ing, and commenting used to only be reachable from the profile page's own inbox preview — `/lobangs` itself was a strictly read-only feed; a report ("Lobangs page should show all interactions") confirmed that read as a bug, not a deliberate scope cut. Both surfaces now share one `LobangReactions` component (the thread itself is a nested `LobangThread`, styled as a light "popout" card when it sits on the ember sent-bubble background), so the two can't drift apart again the way they just had. A sent lobang's own bubble on `/lobangs` shows the same thread, read-and-postable, plus "· liked" inline with the timestamp when the recipient has liked it — the sender is a participant in their own send's thread now, not a bystander reading a one-shot reply back. A received lobang used to be reachable only by scrolling well down "You" — it now surfaces two more places the moment it's unseen: a card near the bottom of Home (same "worth surfacing, not worth interrupting" spirit as the availability/add-to-home-screen cards above it), and a "Lobangs" tab on Places itself, right where "where should we eat" actually gets decided, showing each one as the same place card every other list here uses, the sender's note in its "why" slot. Both surfaces resolve themselves the moment the lobang is actually viewed — "You" 's own inbox stays the one place to browse full sent+received history and dismiss individual ones. |
| **Places** | On a plain, unfiltered browse (no active search or cuisine filter), two personal-suggestion rails sit above the main list — "Quick & nearby" (your top-ranked picks, closest first) and "New to try" (top-ranked picks you've never personally logged a visit to) — replacing `/suggest` as a standalone destination (UX review log #6; the old page is retired, but `/api/suggest` itself lives on, powering these rails and a Jio's own ballot-builder). That ballot-builder is the same "Location" chip + filters + chip-wall panel in two places now — the create-a-Jio form, and a Jio's own detail page once it already exists ("Add a place," open to the host and every invitee, not host-only) — sharing one `SuggestFilterControls` component so the two stay in lockstep rather than drifting into two different experiences. The post-creation panel now explicitly passes the Jio's own `office_id` to `/api/suggest` — it used to fall through to `/api/suggest`'s own default resolution, the *viewer's* own `default_office_id`, so the host and an invitee with a different office preference could see two different candidate pools for the exact same Jio, and a viewer whose own preference didn't match wherever the Jio actually was could end up with a walk-time-filtered pool narrowed to almost nothing — indistinguishable from the "Try:" randomizer being stuck, no matter how many times it's re-rolled, since it really was drawing from the right office's places but the wrong one's walking distances. When the office is already right and the leftover pool genuinely is tiny — a long-running standing rotation, or a group's accumulated blocklists/dislikes, can leave almost nothing "not already an option" within the active filters — the panel now says so plainly (a small note under the filter row, once "Try:" plus "Suggested" between them turn up two or fewer distinct places) rather than leaving a handful of unchanging chips to read as broken. A real, deeper bug turned up right behind that note, on a group Jio specifically: `groupRecommend` computed each member's own personal ranking already truncated to the caller's own `?limit=` (8, from this panel) *before* intersecting "must survive every member" across the group — so a place merely ranked outside one member's own top-8 (nothing to do with an actual exclusion — just a slightly worse walk/budget/rating fit for that one person) silently vanished from the *group's* eligible set entirely, even on a Jio whose true group-eligible pool, computed properly, ran into the dozens. A long-running recurring Jio's standing list eating most of that artificially tiny top-8 then left "Try:"/"Suggested for the group" with only one or two leftover places, forever, no matter how many times a filter was loosened or the randomizer re-rolled. `groupRecommend` now ranks each member without a limit and only truncates the combined, group-averaged result at the very end; the suggestion endpoint's own randomizer pool needed the matching fix (it also draws from the group's already-truncated top-N for its "surprises," rather than recomputing an unlimited group ranking the way personal mode already did) so "Try:" can actually reach every genuinely eligible place, not just whatever's left of the top 8. The creation-time fetch is capped at 8 suggestions rather than a fixed 15, tight enough to read as roughly five rows of chips instead of pushing the rest of the form below the fold. The post-creation panel deliberately does *not* auto-insert into the ballot the way the old "Suggest 3" button used to (silently inserting picks with zero confirmation, and deleting anyone's previously-suggested-but-unvoted options on a re-roll) — every suggestion, including the "Try:" randomizer, is tap-to-add through the same `POST /api/events/[id]/options` a manual search-and-add already uses, so nothing lands in the ballot without someone actually choosing it. The randomizer itself shows up to 3 picks at once rather than one (`surprisePicks`, repeating the same epsilon-greedy `surprisePick` draw against a shrinking pool so nothing repeats), each its own tap-to-add chip, all drawn from — and so already respecting — the exact same filtered/ranked list "Suggested for you" uses; one shared "↻" re-fetches a new set of (up to) 3. Both panels can filter by cuisine and by "On Foodpanda"/"On Grab" presence now, on top of area/budget/"new to you only" — `/api/suggest` takes `cuisines`, `hasFoodpanda`, `hasGrab` the same way `/api/places` does. Cuisine here is a "Cuisine (N)" dropdown of checkboxes rather than a chip row — a horizontally-scrolling strip of every cuisine either overflowed illegibly or wrapped into several rows depending on how many were selected, where a fixed-height dropdown button holds its size regardless — and every pill in the row, including the budget `<select>`, shares one flat background rather than the select rendering as a separate white island against the row's cream card. The dropdown panel measures its own trigger button against the viewport on open and hangs off whichever edge keeps it fully on-screen (an iOS report: a fixed left-aligned panel ran off the right edge whenever the button itself landed in the second half of a wrapped row) rather than assuming a fixed side. The randomizer's reroll button is pinned to the row's own right edge (`ml-auto`) rather than placed immediately after the place chip — its screen position then depends only on the row's own stable width, never on how long the current pick's name happens to be, so a re-roll's shorter/longer name can never reflow a second tap meant for reroll onto the place chip instead (an iOS report: "accidentally click the place when trying to randomise"). It's also sized as a real, visible 44px circle rather than an invisible expanded hit-area over a smaller glyph — `.tap-target-text`'s negative-margin trick (UX review log #2) is meant for a plain text link with no chrome of its own; applied to this bordered/filled button, that same negative margin shifted the *visible* circle up and left of its actual slot, reading as a stray shape floating past the card's edge rather than a small control sitting in its own row. The chip itself just reads "📍 Location" — office-relative is the implied default rather than spelled out in the collapsed state ("Anywhere near office" read as a stale, over-explained label once "Location" said the same thing in one word) — and tapping it opens a picker with two tabs, station search over a static ~180-entry MRT/LRT dataset (sourced from the Singapore Open Data Portal, not hand-typed) or drop a pin on the existing Leaflet map — either resolves to a plain `{lat, lng}` that overrides `/api/suggest`'s usual office resolution for that one request and re-fetches, with a tight 6-minute walk cap by default (no equivalent of a "typical distance" around an arbitrary point the way there is for a fixed office; an area search is a "what's actually around here" ask, a narrower bar than the office-relative default — an initial 15-minute placeholder read too loose in practice and was tightened after shipping). The office-relative default itself now also caps at a real distance (`DEFAULT_WALK_MINUTES`, the same 30 min Places/Map's own filter defaults to) rather than being genuinely unbounded — that used to mean a place far enough away that Places/Map would already exclude it by default could still turn up as a "Suggested for you" chip or the "Try:" randomiser pick, which read as a bug rather than a deliberately looser default the moment someone actually saw it happen (a report: being offered a place clear across town). The same suggestion fetch also takes a budget cap ("Up to," the same six-tier `$`–`$$$$$$` scale and "no ceiling above the top tier is a no-op" convention as the Places page's own filter) and a "New to you only" toggle (excludes anywhere the host has Tried — a logged visit, or a Jio actually attended and decided, not just a visit alone) — both hard cutoffs on `/api/suggest` itself (`budgetMax`/`excludeVisited`, the latter now unioning `listTriedPlaceIds` in alongside logged visits), not just a soft nudge from the host's own `user_prefs`, and both apply per-member in group mode too: one member having already tried somewhere, or it costing more than the group's shared cap, rules a place out for everyone, same as an existing cuisine dislike already does. Nothing here is stored: the area is a per-Jio × per-request query parameter, not a field on the Jio itself, so trying a second area (or clearing the chip back to the office default) within the same form session is just another tap and re-fetch, and neither `walk_cache` nor the Jio record is ever touched by it. "New to try" is cuisine-exclude-aware via `?exclude=`, which is what StreakBanner's "Break it" link and Welcome's "no group yet?" card now point at instead. Searchable list with cuisine, budget, walk-time and rating filters, sortable by nearest, highest-rated, newly rated (most recent rating activity, any visit regardless of `is_public`, same as `avg_rating` already blends both — `places.rating_updated_at`, trigger-maintained since migration 021 but never surfaced until now), or rated by your Kaki group — the budget filter runs six tiers deep (`$` under $8 through `$$$$$$` over $100; CHANGES_20260821.md §1 split what used to be one flat "$$$$ over $30" tier into three, since it lumped a $35 lunch in with a $150 dinner), and defaults to no ceiling rather than silently capping at the old top tier — plus a "Kaki favourites only" chip that actually narrows the list, not just reorders it — joined now by "On Foodpanda"/"On Grab" chips, same narrows-not-reorders shape, computed the same way at the API layer (`hasFoodpanda`/`hasGrab` on `/api/places`) — and a badge on the card itself once at least 2 Kaki-member visits back a place's rating, so a lone review doesn't read as group consensus. `FilterBar`'s "Clear" link tracks whether anything has been touched this session rather than a live value-equals-defaults check — the latter made it flicker away mid-drag the moment the walk slider crossed back over its own default value (30), even though the slider visually sits mid-track there and gives no cue that 30 is special; only an explicit Clear (or a fresh page load) hides it again now. `FilterBar`'s own cuisine row is now the same "Cuisine (N)" checkbox dropdown as the ballot-builder's, not a second, separately-maintained horizontally-scrolling chip strip — both share one `CuisinePicker` component, differing only in the trigger button each renders around it (a bordered `Chip` here, a flat pill there) to match their own surrounding row. `FilterBar` itself moved out of the plain browse ("All") tab and now renders once, above every tab — search, cuisine, budget, walk-time and Foodpanda/Grab narrow Want to try/Tried/Favourites the same way they already narrowed "All," computed client-side (`filterPlaces`, mirroring the server-side `applyFilters` demoRepo/supabaseRepo already run for "All") since those three lists are small and already fully fetched rather than paginated. Two things stay "All"/"Not tried"-only, hidden on the other tabs rather than rendered inert: "Kaki favourites only" and sorting "Rated by your Kaki group," both needing a Kaki-scoped rating only ever computed for a server-backed fetch — which "Not tried" (below) is too, so it gets both for free rather than needing its own case. The walk-time slider's own default (30 min) deliberately does *not* apply to Want to try/Tried/Favourites until the slider's actually been touched this session (the same touched-not-value-equals-default flag "Clear" already tracks) — those three were never walk-filtered to begin with, so applying the default the instant the shared bar rendered would have silently dropped a saved place across town with no visible cause. A tab a filter narrows down to nothing gets its own empty state ("Nothing here matches these filters," a Clear-filters button) distinct from that tab's genuinely-empty state (nothing saved/tried/favourited at all). **"Not tried"** sits right after "All" — the exact mirror of "Tried," every active place minus everywhere already logged a visit to or attended a decided Jio at (`listTriedPlaceIds`, same broader-than-visits bar used everywhere else it's read). Unlike Want to try/Tried/Favourites, it's built as a second mode of the same server-paginated `BrowseList` "All" already uses (`?excludeTried=true` on `/api/places`), not a third small unpaginated list — the "not yet been" set is, by construction, close to the size of the whole catalog, not a bounded personal list. Fixing this up also caught a small pre-existing gap in `/api/places`'s own "needs the full set before slicing" branch (shared by "Kaki favourites only," Foodpanda/Grab presence, and now this): it silently ignored `sortBy=rating`/`newly_rated` whenever one of those filters was active, since only a `kaki_rating` sort was ever handled there — now resolved the same way the plain path already does, so every sort option works underneath any of these filters, not just on an unfiltered browse. The old flat All/Saved split grew into six tabs — All, Not tried, Want to try, Tried, Favourites, Lobangs — once "saved" stopped being one idea. Want to try and Favourites are independent personal lists, not mutually exclusive: saving one never clears the other, so a place can be both at once (a `favourites` table alongside the existing `wishlist` one, gated on its own `favourites` feature flag). Want to try can also carry a short free-text reminder — "the laksa," "ask for the corner table" — since the reason you wanted to go is easy to forget by the time you actually do (`wishlist.note`, edited inline via a small "Add a reminder" link under the card, shown back as the card's own "why" text once set). Tried is never toggled by hand: it's derived from what you've actually logged — a visit, or a Jio you attended (hosted it, or RSVP'd "Yes" and it went on to close with a winner) — a broader bar than "you clicked save," and the same bar "New to you only" (Places' ballot-builder filter) now excludes against too, not just logged visits alone, since a Jio you actually went to is just as much "not new" as a review you wrote. A decided Jio only counts once its own `scheduled_at` has actually passed, not the moment it closes — a Jio confirmed for next Friday hasn't been attended yet regardless of how settled the vote is, same "Going to X" vs "Went to X" tense the Jio itself already uses (`EventRow.tsx`); this applies everywhere `listTriedPlaceIds` is read, so a place you're already committed to for an upcoming Jio still reads as "new to you" in suggestions too, correctly, since you genuinely haven't been there yet. Each tab's own count is a plain length except Lobangs, which stays unseen-only, not a running total — viewing it acknowledges every lobang in it the same way opening "You" or `/lobangs` already does, so the number goes back to a plain "Lobangs" rather than sitting there forever once you've actually looked. Lobangs itself is unchanged: received-only, each shown as the same card shape as everywhere else, the sender's note standing in for the usual recommendation reasoning, with a "Not interested" link removing just that one (the sender is never told); a line underneath points at "You" for the full sent+received history. Add places by hand (address or postal code — a postal code found anywhere in a pasted-in address, e.g. one copied straight from Google Maps, is tried first, since OneMap's road/block index can choke on a business-name prefix or unit number that a bare postal code sidesteps), import candidate names from a food blog, or let the daily cron find them on OpenStreetMap. Cuisine tagging draws from a shared, runtime-extensible list (a `cuisines` table, not a fixed set — see Security notes) rather than free-text alone: typing something new under "Other" that doesn't match an existing cuisine offers to add it permanently, visible to every place and every profile's Taste preferences from then on ("No" keeps today's behaviour, a one-off tag on that place only). An admin combine tool (`/admin/cuisines`) folds near-duplicates ("Korean BBQ" / "korean bbq" / "KBBQ") together after the fact, since normalizing on write only catches exact ones. A place's own page has a "View on Google Maps" link next to Directions, and — when set — a "Socials" link right beside it: one optional, freely-editable field (CHANGES_20260821b.md §1, "Socials," not locked to Instagram — Facebook is just as likely for plenty of places), stored as whatever full URL was pasted and labeled/iconed by sniffing its domain at display time ("View on Instagram," "View on Facebook," or a plain "View socials" for anything else). No auto-resolution exists for it the way Google Places' own lookup does — there's no equivalent "search any business by name" API for Instagram or Facebook to build on — so the edit form's empty-state helper is a plain "Search Instagram for {name}" shortcut instead, Instagram-specific by choice even though the field itself takes any platform's link. Without a Google Places API key configured it's a plain coordinate pin, computed client-side, no backend call — that's still the default. With one configured, the app resolves each place to its actual Google listing, best-effort, server-side, at create/edit time (gated on the result actually looking like the same place — name similarity plus a distance check — not just Google's own top result), and the link opens that instead: the restaurant's real page, photos and reviews included, with the coordinate link as an automatic fallback if the match ever stops resolving. A place can also carry a Foodpanda and/or a Grab link — separate fields from Socials, since a place can reasonably have all three at once and the ballot needs to show whichever are set side by side, not just one generic slot. Same shape as Socials in every respect: manual-only (neither Foodpanda nor Grab exposes a lookup-by-business-name API either), stored as whatever full URL was pasted, freely editable by anyone who can edit the place, and shown next to "View on Google Maps" wherever that already appears — the place page, the ballot/standing lists, and both public preview pages. Each gets its own real brand mark (`DeliveryIcons.tsx`), same exception `SocialsIcon.tsx` already makes for Instagram/Facebook — a user needs to recognize the actual app, not a generic stand-in — sourced from Simple Icons (CC0-licensed vector artwork) rather than redrawn or approximated. Grab's own real-world brand has no square glyph the way Foodpanda's panda or Instagram's camera do — its logo is a wordmark — so it reads smaller and thinner than the other icons at the compact size the ballot/standing rows use; that's inherent to Grab's own branding, not a rendering bug. The edit form's Grab field gets the same "Search GrabFood for {name}" empty-state shortcut Instagram's field has (GrabFood's own search-results URL, confirmed working); Foodpanda's field has no equivalent yet — its exact search-URL format couldn't be confirmed from this environment, and a wrong guess baked into a real link is worse than no shortcut at all.

Every active place also has a public preview page (`/p/[id]`, linked from a Share button on the full page) — the app's only page that needs no sign-in, showing name, cuisine, address, best dishes, aggregate rating and the same Maps link (real listing or pin, same rule) to anyone with the link, alongside the same Socials/Foodpanda/Grab links and icons the full page shows when a place has them set (a link the place's own owner chose to make public carries no more sensitivity than the Maps link already does) — the attributed `/l/[token]` lobang link carries all three the same way — with a "Join to see more" prompt into `/login` for everything past that, including the named review list. A place farther than the walk-time filter's own range (past 45 min, the slider's ceiling — Places, Map, and a Jio's place search all cap out at 60 min or less) is correctly excluded from every list view with no filter setting able to surface it; without any indication of why, that reads as the place having silently vanished rather than just being filtered, so its own page — reachable regardless, unlike the list views — says so plainly, distinguishing "past today's default, widen the filter" from "past the slider's ceiling entirely, no filter helps." |
| **Map** | Leaflet map of everything in walking distance, with real walking routes when OneMap is configured. A Kaki-favourite place gets an amber ring on its marker, layered independently of the existing selected/not-selected fill color. Places that geocode to (almost) the same spot — every stall in a food court, a mall's whole food floor — used to stack invisibly under one clickable marker, so only whichever place happened to render on top was ever reachable; `clusterPlaces()` (`src/lib/mapClusters.ts`) groups them onto a ~11m grid instead of exact-equality, and a group of more than one renders as a single, slightly bigger marker labelled with the count rather than a walk time (identical across the group anyway) — tapping it opens a scrollable list of every place there instead of picking one arbitrarily. Two more badges now show on both a single place's popup and every row in a cluster's list, neither personalized to the viewer (unlike "your Kakis," which stays too): "Highly rated" (`avg_rating` ≥ 4.5, gated on at least 2 visits so a lone glowing review doesn't count — same floor the Kaki-rating badge already applies, just without requiring the raters share a Kaki) and "New" (added within the last 14 days, `created_at`) — distinct from Places' own personalized "New to try" rail (never-visited-*by-you*, regardless of how long the place has existed). Walking distance — here, on Places, and in the ranking behind "Suggested for you" — is measured from your own Profile-page Office setting when you've set one (`user_prefs.default_office_id`), not always the same office regardless of who's asking; changing it there updates all three immediately. That "immediately" needed its own fix: `/api/places`/`/api/suggest` resolve the office server-side, so their SWR cache keys never change when it does, and nothing told the client those cached responses were now stale — this page's own header text and office marker (which read the saved preference directly, not through those two routes) could already show the new office while the pins underneath it were still whatever the old one returned. Saving on Profile now explicitly invalidates every cached `/api/places`/`/api/suggest` response so Map, Places, and a Jio's own suggestion rails actually refetch. Previously only the teammate pickers actually read that preference — everywhere distance-related silently used a single fixed office no matter what a person had chosen on their own profile. |
| **Reviews** | Log a visit privately, or share it as a review. Each visit is its own entry, editable later from either of its two entry points — an "Edit" link next to your own review on the place page, or on the profile's own History list (the only place a private, never-shared visit is reachable at all) — reusing the exact "How was it?" form a new visit uses, pre-filled and pointed at a `PATCH` instead. Un-sharing a review back to private is just unticking the same checkbox. "What would you recommend?" is a `<textarea>`, not a plain single-line field — Enter makes a line break there, never submits the form early the way a bare `<input>` would (and the way iOS's own "Go" keyboard button would trigger on one), with either a comma or a line treated as a separator between dishes. Each shared review can be liked — a bare count plus your own toggle state, no "liked by" list — which nudges the reviewer with a throttled push (at most one per review per ~10 minutes) and feeds into a weekly recap push for anyone whose reviews got at least one like that week. A review is public by default now (UX review log #19, a deliberate reversal of the earlier private-by-default behaviour — every review already written flips too, not just new ones), visible to anyone viewing that place unless you tick it back to private; a plain-language note next to the review form says so, right where the choice actually gets made. The recommended-dishes line on each review reads "Recos:" (matching the app's own term for it elsewhere) rather than "Had:", sitting above the review text at the same size but a lighter grey tone so the two don't read as one undifferentiated block. |
| **Saved places** | Bookmark anywhere from any list, either as Want to try or as a Favourite (independent of each other — see Places above), and saving nudges a place up your own suggestions. |
| **Weather** | Checks the NEA two-hour forecast. When rain is likely, the walk penalty doubles and closer places quietly rise. |
| **Metrics** | What you actually eat versus what you think you eat, plus a nudge when you have had the same cuisine three days running. "Your numbers" on the profile falls back to the shared empty-state card, with a link into Places, before any visit is logged. Headlining that section is a rule-based "food identity" card (CHANGES_20260821_combined2.md Item 1) — The Loyalist, The Explorer, The Regular, The Enthusiast, The Connoisseur, Budget Hunter, or a Well-Rounded Eater catch-all, decided by simple thresholds over your existing metrics, never anything trained. It's a locked monthly snapshot, not something recomputed on every visit — a new cron locks in each month's card early the next month, so it can't flicker mid-month, and a share button renders it as a shareable PNG. When Profile's Taste preferences (cuisine likes/dislikes) are set, one extra sentence is appended after whichever archetype was picked, comparing what you actually ate that month against what you said you like — a real chunk of visits to a disliked cuisine, or mostly sticking to a liked one; purely additive, so it never changes which archetype wins, and says nothing at all when no preferences are set or when actual behaviour doesn't clearly lean either way. Each archetype (and each of the two Kaki award slots) gets its own small hand-drawn "pebble" icon — a single-fill organic silhouette (`ArchetypeIcon`/`AwardIcon`), never paired with a legend or tooltip since every place it appears already sits next to the text that explains it. A history strip of tappable month chips sits under the headline once more than one month is locked in — tapping one swaps the whole card (headline, description, icon, and the Share button's target) to that month, without changing which month is "current." A "Why this archetype?" toggle reveals the exact priority-ordered rule checks `computeFoodIdentity()` ran that month, the account's real number against each threshold, stopping at (and bolding) the one that actually matched. Below the 3-visit floor, the card no longer disappears — a `just_getting_started` outline icon plus a small progress line ("2 of 3 visits logged…") replaces the archetype entirely until enough visits exist to lock one in. The four stat tiles below the card are now tappable rather than staying static numbers — "Visits logged" opens the same month-grouped list the profile page used to show inline (`/profile/visits`), "Different places" a by-place breakdown (`?view=places`), "Average rating" a star-count histogram (`?view=ratings`), all three on `/profile/visits`; "Usual spend" is the one exception, linking to `/profile/spending` instead (see **Spending** above) — that page is the only entry point into logging a first expense, so the link can't wait for one to already exist there first, a real bug in an earlier pass where it kept pointing at the old tier-breakdown tab until an entry existed. It links straight into that page's "My tracker" tab (`?tab=tracker`) once a real logged amount exists, or its default "Insights" tab otherwise — never demoting someone already using the personal tracker behind an extra tap, without forcing everyone else through an empty ledger first either. The cuisine chart grows a "Show all N cuisines" toggle once there are more than the 7 it shows by default. `avgRatingGiven`'s histogram and `avgBudgetTier`'s tier breakdown (still available at `/profile/visits?view=spend`, now sharing `BudgetTierInsights` with `/profile/spending`'s own "Insights" tab) are both computed live inside `computeUserMetrics` alongside the numbers it already derived, not a new endpoint. The tile's own number still upgrades once a real logged amount exists — a real per-entry average in place of the place-tier estimate — but with none yet it shows that same estimate exactly as before, just now landing somewhere you can actually do something about it. The profile page itself was getting long as more activity sections accumulated, so "Want to try," "History," and "Past Jios" now show a 3-item peek with a "See all N →" link into their own page (`/profile/wishlist`, reusing `/profile/visits`, `/profile/jios`) instead of the full list inline — `/profile/jios` adds a Closed/Cancelled filter the inline version never had, and factors the "Send lobang" composer into a shared `PastJioCard` so the peek and the full page render the exact same row rather than two copies that could drift. |
...
| **Recurring Jios** | A standing weekly Jio — same place every time (auto-confirmed, no vote needed) or a vote over the same option pool each week. Generates its next occurrence lazily, a few days ahead, when the host loads Home or Jios; invitees are expanded fresh from current kaki membership every time, not frozen at series creation. The configured time is anchored to Singapore's fixed UTC+8 offset explicitly at generation time, not the server process's own local timezone — the write-side counterpart to the display-only timezone bug above: a 12:00 series used to generate at 12:00 UTC (8pm SGT) instead of the intended 04:00 UTC. Editable, not just stoppable — an "Edit" link reuses the same create form, prefilled. Changes to the weekday only ever affect what generates from then on; time, place/mode and invitees also propagate onto an occurrence the series already generated, but only while it's still open and nobody's voted or RSVP'd on it yet — once someone has, that one occurrence is left exactly as it was, so an edit can never invalidate an answer someone already gave. A series also carries its own vote-deadline offset (see Jios above), defaulting to 3h before each occurrence's start; unlike place/mode/invitees, a deadline change propagates onto an already-generated open occurrence regardless of whether anyone's answered yet, since it can't invalidate a response the way changing what's being voted on would. A series can also carry its own notes for invitees, same field and same propagation rule as the vote deadline (notes don't invalidate an answer either) — previously the only field a one-off Jio has that a series had no equivalent for at all, so every generated occurrence's notes silently stayed blank regardless of what the host typed on the series form. **The Jios calendar (`/events`) previews a series' future weeks even before they've actually generated** — since generation only ever happens lazily, a few days ahead, a standing Wednesday lunch three weeks out otherwise shows nothing at all on the calendar, which is exactly how someone forgets it's there and double-books that slot. A hollow, dashed amber marker (distinct from a real Jio's filled status dot) appears on every future date matching an active series' weekday that doesn't already have a generated occurrence; selecting that day surfaces a lightweight, non-clickable-as-a-Jio preview card ("Wed Lunches · 12:00 · standing, not started yet", with an Edit link straight to the series) rather than a real `EventRow` — it isn't one yet, and shouldn't be mistaken for one. Purely a client-side preview over the series' own weekday/time (`activeSeries`, already fetched for the "Recurring" list above it) — nothing is written or generated early just to back this. |
| **Push notifications** | Opt in from "You": get notified when you're invited to a Jio, when someone votes on one you're hosting (throttled to at most one push per event per ~10 minutes), when a Jio you're in is starting in 30 minutes and you haven't voted or RSVP'd, when one you're in gets decided, when someone sends you a lobang, when someone likes a review you shared (same ~10 minute throttle, skipped for liking your own review), and a weekly recap of how many likes your reviews picked up, sent only if that count is above zero. iOS only ever delivers push to an installed PWA, never a browser tab, so the app also nudges toward "Add to Home Screen" after a few visits — dismissible with "remind me later," not a one-shot ask — plus an always-available "Add to home screen" card in "You" for anyone who dismissed that prompt but changes their mind later. Both surfaces' instructions call out that the Share icon isn't always sitting directly in the visible toolbar — some iOS layouts tuck it behind a "•••"/"More" button first, and a user report confirmed people were getting stuck right there without that hint. In `name` mode, a successful install is also followed by an offer to attach an email, since installing the icon is the moment a second, independent signed-in context is about to exist. `manifest.json` also declares `"handle_links": "preferred"` (plus an explicit `"scope"`) so a tapped Jio/Lobang link can open straight into the already-installed app instead of a fresh browser tab, on whichever Chromium-based browser (Chrome, Edge) has shipped support for it — still rolling out unevenly across browsers as of this writing, and a no-op, not a break, wherever it isn't recognized yet. iOS has no equivalent at all: Safari always opens a tapped link in itself (or whatever app the link was tapped from), never into an already-installed home-screen icon, and that icon runs in its own storage partition entirely separate from Safari's — a sign-in done in one never carries into the other except for a one-time cookie copy Apple performs at the exact moment "Add to Home Screen" is used, which is why installing while already signed in (rather than the other way around) is the only way that first session carries over. Since iOS can't be told to hand a tapped link off to the icon automatically, `HomeScreenNudge` asks instead: a dismissible banner on the signed-out `/e/[token]` and `/l/[token]` previews (where a link shared through WhatsApp/Telegram/etc. actually lands) reading "Already added Jio to your Home Screen? Skip this browser — open the icon there instead," shown only on iOS and only when the current view isn't already the installed, standalone app (reusing `InstallPromptProvider`'s own `platform`/`standalone` detection). Android/desktop installs share cookies with their installing browser, so this repeat-sign-in problem — and the banner — doesn't apply there. The service worker behind the installed icon (`public/sw.js`) never caches the API — stale lunch data is worse than none — but does cache the page shell itself, network-first with a 2.5s timeout: reopening the app on a slow or dead connection shows the last good copy instantly rather than a blank/black splash screen for however long that one request takes, refreshing in the background once the real response lands. |
| **Jio reminders** | A separate, configurable "starting soon" reminder (CHANGES_20260821c.md §1) — not the fixed 30-minute non-responder nudge above, which only ever pushes people who haven't voted or RSVP'd. This one is for anyone confirmed going (RSVP "Yes"), whether or not they've already voted, at a lead time you actually choose. "You" carries the on/off default and the default lead time, right below the push toggle; a Jio you're confirmed for shows its own reminder card, collapsed to "using your default" with a "Change for this Jio" override that only that Jio ever sees. Fires once per person per Jio, same one-shot idea as the fixed reminder above — reopening a Jio for voting or changing your RSVP doesn't matter to it either way. The cron route (`/api/cron/event-reminders`, hit by an external scheduler — see Free-tier realities) claims a reminder as sent *before* attempting the actual push, so a send that never reaches anyone (a transient error, every subscription for that user being dead) used to burn the one-shot claim with nothing ever delivered and no way to retry. `sendPushToUsers` now reports back exactly which users it actually reached; anyone the cron's batch didn't reach — including a whole event's worth, if fetching its push targets itself throws — gets un-claimed (`unclaimReminders`) so the next scan picks them back up, rather than one bad send silently and permanently losing that reminder. |
| **Report a problem** | A "Report a problem" row in "You" (UX review log #17, rescoped from a full help/FAQ page — the vocabulary glossary and auto-close explainer that idea called for needed facts nobody had to hand) opens a small form: a category ("Something's not working," "A place's info is wrong," "Something else") and free-text details. Reuses the exact report mechanism already powering a place's own "Report an issue," generalised with a category field rather than standing up a second reporting system — see Admin above for where these land. Filing one (from here, or Home's "Give feedback" suggestion card) pushes every admin who hasn't muted it — a dedicated `notify_admin_reports` toggle stacked on top of the master push switch, in "You" 's own Admin tools block ("Report & suggestion pushes," on by default), the same "per-type mute on top of the master" shape Jio reminders already use; the reporter is never notified of their own submission even if they're an admin themselves. |
| **Give feedback** | A standing card at the very bottom of Home — "Got an idea for Jio?" — for ideas and suggestions specifically, distinct from "Report a problem" above (that one's for something broken; this is for "you should build..."). Shares the exact same pipeline rather than a second one: `/api/reports`, the `general_reports` table, and the admin moderation queue, just its own `suggestion` category (migration 074 widened the table's category check) so it doesn't get mislabelled as a bug report — `ReportProblemPanel` deliberately never offers this category itself, keeping the two entry points visually separate while sharing the backend. No category picker on this card — every submission here is a suggestion by construction. The subtext also promises something concrete: "Genuinely good suggestions get an in-person coffee, on us." Same admin push as "Report a problem" (see below), muted the same way. |
| **Admin** | Moderation (place reports plus general "Report a problem" reports — see below — side by side on the same screen, filterable by type rather than a separate inbox for each; a general report resolves one at a time, unlike a place's reports which batch-resolve together, since there's no place to group a general one by), block/unblock, an analytics dashboard split into seven tabbed views — Overview (funnel, growth, Jio outcomes), Users, Places, Social, Moderation, Wishlist, and Performance — office management (add, edit in place, or remove; every Jio, walk-time filter, and route default that isn't given an explicit office falls back to the very first office ever created, so moving to a new building means editing that one rather than adding a second office alongside it, which changes nothing on its own), and an accounts screen for merging duplicate identities (auto-surfaced by shared name, or search any account) and issuing recovery links — all reachable from "You", no dedicated nav icon, since admins are the one group that needs it least often. Merging is deliberately two-step, since it's irreversible: "Confirm merge" stays disabled until "Preview" has actually loaded (what will move, what's kept), and — since a disabled button gives no feedback of its own, which read as "I pressed merge and nothing happened" rather than "load a preview first" — a plain-text hint next to the buttons now says so directly while it's still disabled. A genuine Preview/Confirm-merge failure now surfaces right next to that group's own buttons, not a single page-top banner easy to scroll past. Confirming that fix in production turned up the actual bug behind an "I previewed, then confirmed, and the duplicate never disappeared" report: retiring the merged-in account is the one step needing the service role (`serviceClient.ts`) — deleting an `auth.users` row isn't something any RLS-scoped call can do — and a failure there (a stale/misconfigured key, the row already gone) used to be swallowed as a server-log-only warning even though every row of real data had already moved successfully. `mergeUserAccounts` now reports that failure back through the merge response instead of silently keeping it server-side, so the admin sees exactly why an account that should be gone is still sitting there, rather than a merge that looks like it did nothing at all. That warning's own wording needed a second pass once it actually reached production: Supabase's Auth Admin API can fail with an HTTP error status but an empty JSON body, and `@supabase/auth-js` falls back to `JSON.stringify()`ing that empty body for `.message` — the literal, uninformative string `"{}"` — so the merge warning now reads `.status`/`.code` off the same error instead of trusting `.message` alone, and merging 2+ stale accounts into one keeper in a single request (each capable of producing its own delete-failure warning) now lists them one per line rather than running them together into one garbled, duplicated sentence. That diagnostic upgrade then found the actual root cause: `app_daily_visits`/`action_events` (Daily Activity Log, added later than `merge_user_accounts` was last touched) both have a hard `references auth.users(id)` with no cascade, and every signed-in page view writes an `app_daily_visits` row — so any real account being merged away almost always had one, which blocked its own `auth.users` row from ever being deletable at all, no matter how many times "Confirm merge" was pressed. `merge_user_accounts` now reassigns both tables to the surviving account (merging same-day visit counts rather than dropping one side's history on a collision), so a duplicate account's data — and the account itself — actually goes away. The Performance tab now includes an in-app DAU/WAU/MAU trend (distinct users doing anything — voting, hosting, logging a visit, saving a wishlist item, adding a place, or filing a report — bucketed by day/week/month) alongside the existing Vercel/Supabase link-outs, which still cover Core Web Vitals and platform-quota metering this app's own database can't query. Page views and unique visitors by page moved in-app too — "Page views by tab" (also on the Performance tab) is a table per day, one row per `BottomNav` section (Home/Jios/Kakis/Places/Map/You, plus Admin/Other for everything outside the bar) with PV (total views) and UV (distinct signed-in visitors), over a fixed trailing 14 days independent of the date-range picker, newest day expanded by default. `AppVisitTracker`'s existing per-route-change beacon now also posts the pathname it fired from; the ping route buckets it into a tab (`tabForPath`) and upserts a `page_views_by_tab` row (user, day, tab) alongside the pre-existing per-day tally, same increment-on-conflict shape as that table — unlike that one, this one's `user_id` FK is `on delete cascade` from the start (migration 087), having just been bitten by the alternative in the paragraph above. `merge_user_accounts` carries a merged-in account's page-view-by-tab history onto the survivor the same way, merging same-(day, tab) counts rather than dropping one side's on a collision. Overview also carries a real step funnel — invited → responded → voted → attended → reviewed, with real drop-off percentages, a per-week trend for each step, and a cohort table grouped by signup week — scoped to every Jio that actually closed with a winner in the window, distinct from the same-day activity snapshot above it. The Places view's top-rated/most-visited lists are now click-through: opening a place shows who's visited (ranked), a weekly rating trend rather than just the single current average, wishlist-save and lobang-mention counts, and how well the place's cuisine/budget lines up with the people who actually go there. The Users view carries a leaderboard scored by a composite engagement signal (hosting, voting, RSVPing, logging visits, writing public reviews, sending lobangs) — equal-weighted by default, but adjustable from an on-page settings form since the next admin may want to weight differently — six rule-based segments (Power hosts, Active voters, RSVP-only lurkers, Reviewers, Dormant, New & active; not a partition, a person can land in more than one or none), and the same click-through pattern into a per-person drill-down: full visit history, cuisine breakdown, regulars, Jios hosted, Kaki memberships, lobangs sent/received, last-active date, and lifetime RSVP responsiveness. A shared date-range picker (7/30/90/180 days or 1 year, via a `?days=` param that persists across every tab) replaced the dashboard's old fixed 90-day window everywhere. Overview also carries a segment filter that re-slices Jio Outcomes and the real funnel to just the Jios hosted by one segment's members (Growth stays unfiltered — filtering brand-new signups by an activity segment they haven't had time to earn isn't a coherent question), and the Growth view's "new users" chart now expands to show who actually joined each day, not just the count. An "Export CSV" button sits on the dashboard's highest-traffic tables and charts (Growth's four series, the Users leaderboard, Places' two ranked lists, Wishlist's saves and most-saved places) — plain client-side generation from data already on the page. The dashboard's two chart primitives — a sparkline and a horizontal-bar distribution — print their key numbers as visible text by default rather than only on hover: a sparkline shows its date range plus the peak and latest values always, with a tap-or-hover readout per bar for the rest; a distribution bar shows a 0→max scale and an "N total" line, in one consistent color rather than cycling through decorative hues that don't encode anything. Overview also carries "Recent entrants" — who visited the app on each of the last 7 days, day by day, always the trailing week regardless of the date-range picker (Daily Activity Log). Every signed-in page view is tallied server-side, per calendar day — this is "who came back," not "how many pages were requested," which is what the tab-level PV/UV table above is for. A per-person drill-down's own "Daily activity" card shows the same person's last 30 days: a day they visited with nothing else logged still appears, empty; a day they never opened the app at all doesn't appear. Instrumented actions (hosting, voting, RSVPing, logging a visit or review, sending a lobang, saving a place, adding a place, creating a Kaki, filing a report, flagging a place) show up under whichever day they happened, alongside the plain page-view count. |
...
- **Colour is never the only signal** (UX review log #8). The Profile
  taste-preference chips add a heart/dot/X glyph alongside their existing
  sage/grey/ember tint — colour unchanged, purely additive, so a
  colour-blind user can tell their own "like" from their own "dislike";
  the legend below them gets the same three icon+label pairs instead of
  two bare coloured dots. The Jios calendar's open/closed/cancelled status
  dots get `role="img"` + `aria-label` + a matching `title` (removed from
  `aria-hidden`) so a screen-reader gets the plain state name and a mouse
  user gets a hover tooltip, from the same string — no visual change.
- **Every `role="dialog"` popover gets real dialog mechanics** (UX review
  log #9) via a shared `useDialogFocus` hook (`src/lib/useDialogFocus.ts`):
  focus moves to the dialog's primary control on open, Tab/Shift+Tab are
  trapped inside while it's open, Escape closes from anywhere inside, and
  focus returns to exactly whatever had it before the dialog opened on
  close. Applies to the add-to-home-screen prompt, the recovery-link
  nudge, the QR code popup, and the decided-Jio celebration — a generic
  hook rather than moving all four to the native `<dialog>` element, since
  they range from a full-screen overlay to a backdrop-less bottom
  snackbar and re-styling every one for `<dialog>`'s top-layer rendering
  risked layout regressions this pass couldn't fully re-verify visually.
- **Loading skeletons match their real content's shape**, not one generic
  block (UX review log #14). The Jio/Place/Kaki detail pages each get
  their own multi-block skeleton (`SkeletonJioDetail`/`SkeletonPlaceDetail`
  /`SkeletonKakiDetail` in `ui.tsx`) proportioned to that specific page's
  real major sections, rather than sharing one uniform ~220px block
  against pages that actually run 1,500–2,500px; Places' browse/Saved/
  Lobangs-tab rows now use a skeleton row sized to the real ~110px
  `PlaceCard` rather than an 80px default; and the standalone Lobangs
  feed's skeleton alternates left/right alignment to mirror the real chat
  layout instead of six identical left-aligned bars.
- **A broader type scale** (UX review log #16): `text-xs`/`text-sm` are
  metadata/secondary-only now, `text-base` (16px) is the new default for
  primary reading text — place names and Jio titles in every list row,
  and a review's own body text — moved up from `text-sm`, and a detail
  page's own title (Jio, Place, Kaki) moved from `text-2xl` to `text-3xl`,
  filling the gap between a 24px section header and 36px+ hero text.
  Tailwind's own default scale already matched the target shape exactly
  (no new tokens needed) — the fix was widening *use* of steps that
  existed but sat underused, not inventing new ones.
- **A small illustration/motif system** (UX review log #18), four marks
  in the pebble/JioMark family for the four highest-visibility empty/
  celebration states (`src/components/brand/motifs.tsx`), built from
  JioMark.tsx's own real traced path geometry rather than an
  approximation: a hollow dashed pebble outline for "No Jios yet" and
  "You are not in any groups," the same outline stretched toward a pin
  silhouette for "Nothing saved yet," the three real ember pebbles
  (no centre dot) for the everyday "Decided" banner — a static echo of
  the resolved-celebration animation already locked in #25 — and two
  overlapping solid pebbles for the empty Kakis state, extending the
  pebble-as-person treatment already scoped to Kaki Members in #24.
  `EmptyState` grew an optional `icon` prop for this; every other empty
  state in the app is unaffected.
- **Non-idempotent toggles are guarded against a fast double-tap** (UX
  review log #10): the wishlist "Want to try" button and the `SaveButton`
  bookmark used on every card get the same disable-on-tap + optimistic-
  update + rollback-on-error pattern RSVP/vote already used, so two taps
  landing before the first response returns can no longer race and cancel
  each other out. Cancelling a recurring series is one-way, not a true
  toggle, so it gets disable-on-tap plus error catch/surface only — there
  was nothing to roll back to.
- **"Event" doesn't leak into user-facing copy anymore** (UX review log
  #11) — that's the internal database name for what the UI always calls
  a "Jio"; every user-facing string that said "Event" now says "Jio," and
  a couple of raw/technical error strings (a bare "Expected a JSON body,"
  a generic "Something went wrong") were rewritten in the same warm,
  specific voice as the rest of the app. The Kakis nav tab, which read
  "Kaki" (singular) against the page it opens ("Kakis," plural, matching
  every other section), is now "Kakis" too — the same one-word
...
Valid keys: `events`, `kakis`, `wishlist`, `favourites`, `lobangs`, `blogImport`,
`discovery`, `weather`, `map`, `metrics`, `roulette`, `reviews`, `offices`.
...
**The vote-deadline sweep (below) needs the same setup, as a third job on
the same external scheduler.** `GET /api/cron/vote-deadline` is likewise
left out of `vercel.json` for the same reason — a host-set deadline needs
checking every few minutes to actually close on time, not once a day.
Same bearer token, roughly every 5 minutes. Without it, a Jio's vote
deadline is fully settable and shown everywhere, but the Jio itself never
actually force-closes when it passes — nothing else in the app closes a
Jio automatically any more (see `readyToClose` below), so without this
cron a Jio with a deadline set just sits open past it, same as one with
no deadline at all.
...
`listAllVisitsForCron`/`listAllPlacesForCron`/`listAllKakisForCron`/
`listAllUserPrefsForCron` (the last one added later, to compare each
account's Profile Taste preferences against actual visit behaviour for the
optional preference-alignment note — same `authenticated`-only,
owner-scoped RLS on `user_prefs` as the others, same reason it needs the
service-role client). That fix mattered: the cron used to gather its own
input data — visits, places, Kaki membership — through the ordinary
per-request client (`listVisits`, `listPlaces`, `getKaki`), every one of
which is `authenticated`-only under RLS, same as `review_likes`. A Vercel Cron invocation has no session at
...
`google_place_id` needed to stay off the grant, 060 for `socials_url`
(CHANGES_20260821b.md §1), and 083 for `foodpanda_url`/`grab_url`).
Skipping that step reproduces the exact bug this migration exists to
prevent, just for the new column instead.
...
**The Daily Activity Log's own two functions are `SECURITY INVOKER`, not
`SECURITY DEFINER` — the opposite of every admin-analytics function above.**
`track_daily_visit` (076) and `log_action` (076) only ever read or write
`auth.uid()`'s own row in `app_daily_visits`/`action_events`, which RLS
already permits directly (`app_daily_visits_insert`/`_update`, `action_
events_insert`, all `user_id = auth.uid()`) — no elevated privilege is
needed to make the increment-on-conflict upsert atomic, unlike
`get_admin_analytics` and friends, which genuinely need to read across
every user and so have to earn that with their own explicit admin check.
`logAction` (`lib/actions.ts`) wraps the repo call in a try/catch that only
logs and returns — same "a courtesy on top of the real write, never a
condition for it succeeding" shape `sendPushToUsers` already established
...
**Full-consensus auto-close (CHANGES_20260821_combined.md Part 2) used to
be a fifth reason the service-role client existed — it no longer is.**
`maybeAutoCloseEvent` ran synchronously inside the RSVP and vote routes
right after each write, closing a Jio itself, no host action required,
the moment every *current* participant had RSVP'd and everyone going had
voted. A real bug report — "Jio keeps closing whilst new members trying
to add their votes in" — found the flaw: a Jio anyone can still join via
its own share link (or, kaki-linked, just by being a member) can never
have a genuinely final population, so "everyone's answered" is a moving
target. Someone opens the link, joins, votes, and if everyone *already*
joined had answered, the Jio closed right then — locking out whoever was
about to open that same link a moment later. There's no way to tell
"everyone who'll ever join has voted" from "everyone who's joined so far
has voted" from inside that check alone, so closing on it automatically
was fundamentally unsafe for any Jio someone else could still join.

The same condition is still computed — every current participant
answered, every "yes" backed by a fresh (non-stale) vote — but only ever
as a read, `EventDetail.readyToClose` (`computeReadyToClose`,
`src/lib/voting.ts`), recomputed on every `getEvent`. It's surfaced as a
prompt on the host's own Jio page ("Everyone's answered — ready to
close?") right under the share link, not a background job — the host
decides when it's actually done, same "Close with the vote" action the
page already offers further down, just harder to miss. `notifyHostOfVote`
(vote route) and its RSVP-route counterpart both switch their push copy
to the same prompt once a write makes this newly true, reusing the
existing per-event vote-push throttle rather than a new one. Closing
itself now only ever happens through the host's own explicit request
(`closeEvent`, still host-only RLS, no service role needed) or the
vote-deadline sweep below — both already had a legitimate path to close
without racing a still-growing guest list, since a deadline is a fixed
point in time, not "whoever happens to have joined so far."
...
```bash
npm test          # 902 tests across 71 files
npm run typecheck
npm run lint
```
...
| File | Covers |
|---|---|
| `recommend.test.ts` | Every scoring component, exclusions, ranking, boosts, group mode; `rankPlaces`'s two hard-cutoff filters for the Jio ballot-builder's suggestion list — `budgetMax` (excludes anything tiered above it, a no-op at the top tier) and `excludeVisited` (excludes anywhere already logged, rated or not) — and that `groupRecommend` passes both through per-member, so one member's own visit or budget ceiling rules a place out for the whole group; that `groupRecommend`'s own per-member ranking ignores the caller's `limit` (a place ranked outside one member's own top-N, but excluded by nobody, still belongs in the group's eligible set) and only the final combined result gets truncated; and that composing it with `surprisePicks` off a `limit`-capped ranking loses genuinely eligible places a truly unlimited ranking can still reach |
| `areaScopedSuggest.test.ts` | Area-Scoped Suggest — `listPlaces`'s `officeId` filter widened to accept an ad-hoc `{lat, lng}` point: computes haversine directly from that point (even one sharing the same coordinates as a cached office, to rule out an accidental cache hit), never consults `walk_cache` for it, and leaves office-relative resolution (no `officeId`, or the office id string) behaviourally unchanged; the static `STATIONS` dataset is non-trivial, within Singapore's bounds, has no duplicate names, and carries every real serving line for known interchanges (Dhoby Ghaut, City Hall, Promenade, Raffles Place) |
| `blogImport.test.ts` | HTML extraction and the full SSRF matrix |
| `eventAdditions.test.ts` | Who can add, remove, invite, vote and close; setting a place-specific note is restricted to whoever added that option (not the host, unlike removing), rejected once closed, and a blank note collapses to `null`; joining via an invite link makes a stranger a real invitee, not just visible; a genuine first-timer's `listEvents` is empty right up until their first join; the vote-push throttle window; the starting-soon reminder's timing, one-shot firing, and non-responder targeting |
| `eventReminders.test.ts` | The configurable per-Jio "starting soon" reminder (CHANGES_20260821c.md §1) — a genuinely different feature from the row above's non-responder nudge: per-Jio override set/read/clear; `listAndClaimDueReminders` firing only for confirmed-going (RSVP `yes`) attendees within their effective lead time (override, else their `user_prefs` default), one-shot per (event, user), skipping cancelled Jios, already-passed Jios, and anyone with reminders turned off — including someone with no `user_prefs` row at all, who still gets the column defaults rather than being silently skipped; `unclaimReminders` reverting a claim so the same (event, user) pair is due again, touching only the exact pairs given (a sibling's already-delivered claim stays claimed), and a no-op for a pair with no reminder-state row at all |
| `voteDeadlineReminder.test.ts` | `listAndClaimVoteDeadlineReminders` (088_vote_deadline.sql) — a different one-shot shape from the row above: event-level, not per-user, since it's a single courtesy nudge per Jio rather than a configurable per-recipient lead time. Claims within the lookahead window; resolves only pending participants (excludes whoever's already voted or declined); doesn't re-fire on a second sweep; skips a deadline outside the window, already passed, unset, or on a still-polling Flexi Jio; re-fires once a host pushes an already-claimed deadline back out into the future |
| `autoClose.test.ts` | `EventDetail.readyToClose` (formerly `maybeAutoCloseEvent`'s own auto-close condition, CHANGES_20260821_combined.md Part 2 — now a read-only prompt, not a close, per the bug report below) — `false` while any participant hasn't responded or answered "maybe", `false` while any confirmed-yes participant hasn't voted, `true` once every condition holds (the Jio itself stays `open` regardless — nothing closes it), `true` with no winner in view when everyone declines, the host has no auto-confirm exception, `false` on a still-polling Flexi Jio, an already-closed event, and a cancelled event; a new place added after a yes-voter's ballot keeps it `false` entirely — bug report item 4 — until everyone's recast past it, and `getEvent`'s own stale-vote flag flips true then clears the same way; a new joiner (via `joinEventViaInvite`) flips an already-`true` Jio back to `false` — the exact "Jio keeps closing whilst new members trying to add their votes in" report this replaced, now impossible since nothing ever closes on this by itself; `closeEventsPastVoteDeadline` (088_vote_deadline.sql) — closes an event past its deadline even with unresponded participants (confirming the deadline is a real fallback, not just theoretical) or a stale ballot (the deadline still wins regardless of staleness), no-op before the deadline / with no deadline set / on a still-polling Flexi Jio / on an already-closed or cancelled event, `null` winner when nobody voted by the deadline |
...
| `metrics.test.ts` | User and group statistics, cuisine streaks; `computeUserMetrics` also buckets ratings into a by-star histogram and visits into a by-budget-tier breakdown, the data behind the "Average rating"/"Usual spend" stat tiles' own detail pages; `computeKakiMetrics` weighs each member's cuisine taste and average spend equally regardless of visit volume (a member with 50 visits doesn't outweigh one with 3), and names a "Trailblazer" — the member with the most places nobody else in the group has been to, `null` when everyone's only ever been somewhere shared; also returns every member's full ranking for each of the three awards, sorted highest-first, leaving a member out entirely once their value is 0 |
| `kakiCrown.test.ts` | `resolveKakiAwardCrown`'s home-kaki rule (Log 6 Part B): a normal crowning when the winner isn't #1 anywhere else; crowns the winner (with a streak count) when this is their best group among several they lead; passes the crown to the runner-up, with a note naming the real leader and their actual home group, when a different group is their best; `null` when the crown would pass but there's no runner-up to receive it; the earliest-created-group tie-break when two groups tie on raw value — matches the log's own worked example (Priya/Weekend Foodies/Office Lunch Crew) exactly |
...
| `voting.test.ts` | Borda count, partial ballots, tie-breaking; `isVoteStale` — never stale with nothing changed, stale once the vote predates the last options change, fresh once cast after it, a missing timestamp treated as stale rather than given the benefit of the doubt |
...
| `generalReports.test.ts` | "Report a problem" / "Give feedback"'s shared `general_reports` pipeline — lands pending, surfaces in the queue oldest-first, an admin resolves one report without touching others, only an admin can resolve, and the `suggestion` category (Home's `FeedbackCard`) is accepted and queued the same as any other |
| `adminReportNotifications.test.ts` | `listAdminReportRecipients()` — the read that decides who gets the "someone filed a report/suggestion" push: includes an admin by default, excludes one who has muted it via `setNotifyAdminReports`, includes them again once turned back on |
| `lobang.test.ts` | Targeted sends, group-send member snapshotting; public sends — no recipient rows created, excluded from the sender's own Sent list, resolve through `getPublicLobang` with the real sender name and note (including `foodpanda_url`/`grab_url`, same as `socials_url`), `null` for an unknown token or once the place is no longer active; `sendLobang`'s resolved `recipient_ids` (teammates, every Kaki member but the sender, empty for a public send) that the API route pushes to directly; heart toggles on/off and refuses a non-recipient; the shared comment thread — a posted comment is visible via both sides' own lists, the sender can post back into it too (not one-way), a group send's thread is shared by every recipient and the sender, refuses an empty comment, and refuses reading or posting for anyone who isn't the sender or a recipient |
...
| `placeEditing.test.ts` | Any signed-in user can edit a place; `status` cannot move through a plain edit; `socials_url`/`foodpanda_url`/`grab_url` round-trip through create/update same as every other optional field, and clear back to `null` |
...
| `rescheduleAndEditWinner.test.ts` | Host-only date/time correction at any status short of cancelled, finalizing a still-polling Flexi Jio's date the same way confirming a candidate does; host-only rename, available at any status including cancelled, trimmed, rejecting an empty title; host-only winner-place correction, closed-only, accepting a place that was never a voting option, stamping `winner_corrected_at`; host-only reopen-for-voting, closed-only and only while still ahead of its scheduled time, clearing the winner (and a prior correction's stamp) while leaving existing ballots untouched |
...
| `recurringSeries.test.ts` | Recurring-series date math (lookahead window, no double-generation), fixed vs. voted occurrences, fresh kaki-membership expansion per occurrence, each generated occurrence inheriting the series' own vote-deadline offset and notes, and a deadline or notes change propagating onto an already-generated open occurrence even after someone's voted (unlike place/mode/invitees) |
...
| `mapClusters.test.ts` | Map marker clustering: a lone place stays its own group, exact-coordinate and within-grid places group together, different buildings stay apart; the "Highly rated"/"New" badge thresholds (rating + minimum visits, listing age) |
| `filterPlaces.test.ts` | The client-side counterpart to demoRepo's server-side `applyFilters`, run against Want to try/Tried/Favourites: cuisine (any-match), budget (no-op at the top tier), search (name/address/dish/cuisine, case-insensitive), Foodpanda/Grab presence; the walk-time cap specifically only narrows once `applyMaxWalk` is passed (mirroring the shared `FilterBar`'s own "touched" flag), so a saved place across town doesn't vanish the instant the bar starts applying to a tab that was never walk-filtered before |
| `wishlistFavouritesTried.test.ts` | Places' Want to try / Tried / Favourites restructure: a wishlist reminder note round-trips and can be cleared back to `null`; a note is refused for a place never saved; Favourites toggles independently of the wishlist (a place can be both, or neither, or either alone), hydrates its joined place record; `listTriedPlaceIds`/`listTried` include a logged visit, a closed Jio the user hosted, and one they attended as a guest (RSVP'd yes), while excluding a Jio they were only invited to (RSVP'd maybe/no), one that never closed, or one that's closed but decided for a still-future date, de-duplicating a place tried both ways, and returning nothing for an account with no visits or attended Jios |
...
| `kakiWishlist.test.ts` | The shared Kaki wishlist: a member can add (hydrated with the place record and adder's name) and any current member can remove an entry, not just whoever added it; a non-member can neither add nor remove; refuses a duplicate add of the same place; scoped per-Kaki (one group's list never leaks into another's); an entry disappears once its place is blocked without deleting the underlying row (confirmed: filter, don't prune) |
| `expenseTracking.test.ts` | The personal expense ledger: logs an entry defaulting `logged_at` to today; lists only the calling user's entries for a given month, newest first; paginates 20/page with no overlap between pages; amends an entry in place (confirmed: edit, not delete-only); accepts all six categories (breakfast/lunch/dinner/coffee/snack/other, migration 095); links a place match found while editing and can clear it back off; refuses editing or deleting someone else's entry; an entry survives its source visit being deleted, just unlinked (confirmed: `on delete set null`, not cascade); hydrates `place_name` once an entry is created with a matched place; `getExpenseMonthSummary` totals by category, computes a delta against the previous month (`null` with nothing to compare), and writes a plain-language comparison note against the account's own budget preference; `centsToBudgetTier`'s reverse-mapping of the budget-tier dollar bands |
| `publicPlace.test.ts` | The public place-preview data: only the safe field subset, never `notes`/`created_by`, carries `lat`/`lng`, hides `needs_review` and `blocked` places, computed rating and visit count match the authenticated view; carries `foodpanda_url`/`grab_url` same as `socials_url`, `null` when neither is set |
...
| `foodIdentity.test.ts` | `computeFoodIdentity`'s priority-ordered archetype rules (Loyalist/Explorer/Regular/Enthusiast/Connoisseur/Budget Hunter, the Well-Rounded Eater catch-all, "Just getting started" below the visit floor), including that a dominant cuisine outranks a high rating when both technically qualify; the optional preference-alignment note it appends afterward — a real chunk of visits to a disliked cuisine, sticking to a liked one, no note at all when no preferences are set or when neither clearly leans either way, in every case without changing which archetype was picked; `getArchetypeRuleTrace`'s "why" panel rows — stops at and marks the rule that matched, lists every unmatched rule before it in order, falls through to the last-checked rule unmatched for the well-rounded catch-all; `computeKakiFoodIdentity`'s group vibe headline and award passthrough; `previousMonthKey`'s month rollback, including across a year boundary |
| `foodIdentitySnapshots.test.ts` | `listAllUserIds`/`listAllKakiIds` cover every seeded account/Kaki; `listAllUserPrefsForCron` returns every account's cuisine likes/dislikes; `save*FoodIdentitySnapshot` upserts per (id, month) rather than duplicating, keeps months scoped to their own account/Kaki, and lists most-recent-first |
...
| `utils.test.ts` | `formatTime`/`formatDate` render in Singapore time regardless of the runtime's own timezone; `relativeDayLabel` agrees with the Singapore calendar rather than UTC's right at the UTC day boundary; `socialsHost`/`socialsLabel` (CHANGES_20260821b.md §1) sniffing Instagram/Facebook/other from a `socials_url`, falling back cleanly on an unparseable URL; `instagramSearchUrl`'s keyword-search link; `grabFoodSearchUrl`'s GrabFood search-results link |
...
[data.gov.sg](https://data.gov.sg). MRT/LRT station coordinates (`src/lib/
stations.ts`) from the Singapore Open Data Portal (SODL-1.0), via
[elliotwutingfeng/singapore_train_station_coordinates](https://github.com/elliotwutingfeng/singapore_train_station_coordinates).
