...
3. **SQL Editor** — run every file in `supabase/migrations/` in numeric order,
   001 through 061. They are idempotent, so re-running is harmless.
...
```bash
npm test          # 535 tests across 43 files
npm run typecheck
npm run lint
```
