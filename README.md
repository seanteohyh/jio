...
```bash
npm test          # 839 tests across 69 files
npm run typecheck
npm run lint
```
...
| `metrics.test.ts` | User and group statistics, cuisine streaks; `computeKakiMetrics` weighs each member's cuisine taste and average spend equally regardless of visit volume (a member with 50 visits doesn't outweigh one with 3), and names a "Trailblazer" — the member with the most places nobody else in the group has been to, `null` when everyone's only ever been somewhere shared; also returns every member's full ranking for each of the three awards, sorted highest-first, leaving a member out entirely once their value is 0 |
