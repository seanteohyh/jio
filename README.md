...
3. **SQL Editor** — run every file in `supabase/migrations/` in numeric order,
   001 through 062. They are idempotent, so re-running is harmless.
...
```bash
npm test          # 540 tests across 44 files
npm run typecheck
npm run lint
```
