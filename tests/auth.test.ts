...
describe("auth adapters", () => {
  ...
  it("all declare what the login page should render", async () => {
    for (const [label, adapter] of await adapters()) {
      ...
      expect(typeof adapter.capabilities.attachEmail).toBe("boolean");
  ...
  it("only name mode advertises the email-attach upgrade", async () => {
    for (const [label, adapter] of await adapters()) {
      expect(adapter.capabilities.attachEmail, label).toBe(label === "name");
    }
  });
  ...
  it("refuses unsupported styles instead of throwing", async () => {
    ...
    // Email mode already has a permanent identity — there is no anonymous
    // session to upgrade.
    await expect(supabaseAuth.attachEmail("a@b.com", "/")).resolves.toEqual(
      UNSUPPORTED
    );
    await expect(
      supabaseAuth.verifyAttachEmailOtp("a@b.com", "123456")
    ).resolves.toEqual(UNSUPPORTED);
  });
  ...
  it("attachEmail rejects a blank email before touching the network", async () => {
    const { nameAuth } = await import("@/lib/auth/nameAuth");

    const result = await nameAuth.attachEmail("   ", "/");
